...
setup('authenticate', async ({ page }) => {
  ...
  const googleButton = page.locator('button', { hasText: 'Google' }).or(
    page.locator('[data-provider="google"]')
  ).or(
    page.locator('button[data-clerk-element-id*="google"]')
  ).first();
  ...
  try {
    ...
    console.log('⚠️  No obvious redirect, checking auth state on current page...');
  ...
  for (const indicator of authIndicators) {
    if (await indicator.isVisible({ timeout: 5000 })) {
      console.log('Authentication verified with indicator:', await indicator.textContent());
  ...
  if (!isAuthenticated) {
    throw new Error('Authentication failed - could not verify user is signed in');
  }
